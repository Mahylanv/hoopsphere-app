...
function hasMixte(teams: FirestoreClub["teams"]): boolean {
    ...
    const check = (t: string) => /(^|\W)(mixte|les deux|both)(\W|$)/i.test(normalize(t));
    if (Array.isArray(teams)) return teams.some((v) => typeof v === "string" && check(v));
    if (typeof teams === "string") return teams.split(/[,/|]/).some((p) => check(p));
...
function extractTeamKinds(
    ...
    const pushFromToken = (t: string) => {
        const n = normalize(t);
        if (/(masculin|masculine|masculins|homme|men|male)/.test(n)) out.add("masculines");
        if (/(feminin|féminin|feminine|feminines|femme|women|female)/.test(n)) out.add("feminines");
    };
    ...
    useEffect(() => {
        ...
        if (search) {
            ...
            results = results.filter((c) => {
                ...
                return name.includes(lower) || city.includes(lower) || dep.includes(lower);
            });
        }
        ...
        <SafeAreaView className="flex-1 bg-black">
            ...
            <View className="px-4 pb-2">
                ...
                <View className="flex-row items-center justify-between mb-4">
                    <View className="flex-row items-center">
                        ...
                        <Text className="text-white text-2xl font-bold ml-2">Rechercher un club</Text>
                    </View>
                    <TouchableOpacity onPress={() => setFilterVisible(true)} className="p-2 rounded-xl">
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={<Text className="text-gray-500 text-center mt-10">Aucun club trouvé</Text>}
                    renderItem={({ item }) => {
                        ...
                        return (
                            <Pressable
                                ...
                                className="flex-row items-center bg-[#1a1b1f] rounded-2xl p-4 mb-3 border border-gray-800"
                            >
                                {item.logo ? (
                                ...
                                <View className="flex-1">
                                    <Text className="text-white text-lg font-semibold">{item.name || "Club sans nom"}</Text>
                    ...
                    ListFooterComponent={
                        visibleCount < filtered.length ? (
                            <TouchableOpacity onPress={handleLoadMore} className="mt-3">
                                <Text className="text-center text-orange-500 font-semibold">Charger plus</Text>
                            </TouchableOpacity>
                        ) : filtered.length > 0 ? (
...
function OffersFilter({
    ...
    const [championships, setChampionships] = useState<string[]>(initial?.championships ?? []);
    const [ageClasses, setAgeClasses] = useState<string[]>(initial?.ageClasses ?? []);
    ...
    const applyFilters = () => {
        onApply({ categories, championships, ageClasses, departments, positions, genders });
        onClose();
    };
            ...
            <View className="flex-1 bg-black/50 justify-end">
                ...
                <View className="bg-[#1a1b1f] p-5 rounded-t-3xl max-h-[90%] border-t border-gray-800">
                    ...
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        {/* Département */}
                        <View className="mb-6">
                            <Text className="text-white text-lg font-semibold mb-3">Département</Text>
                            <DepartmentSelect
                                value={departments}
...
function OffersTab() {
    ...
    const [clubsIndex, setClubsIndex] = useState<Record<string, { department?: string; name?: string; logo?: string }>>({});
    ...
    useEffect(() => {
        ...
        const unsub = onSnapshot(uq, (snap) => {
            const idx: Record<string, { department?: string; name?: string; logo?: string }> = {};
            snap.forEach((doc) => {
                const d = doc.data() as any;
                idx[doc.id] = { department: d?.department || "", name: d?.name || d?.nom || "", logo: d?.logo || "" };
            });
            setClubsIndex(idx);
        });
        return () => unsub();
    ...
    useEffect(() => {
        ...
        const q = fsQuery(collectionGroup(db, "offres"), orderBy("publishedAt", "desc"));
        const unsub = onSnapshot(
            q,
            (snap) => {
                ...
                snap.forEach((d) => {
                    ...
                    list.push({
                        ...
                        championship: data.championship || data.championnat || data.league || "",
    ...
    useEffect(() => {
        ...
        // Catégories
        if (filters.categories && filters.categories.length > 0) {
            res = res.filter((o) => !!o.category && filters.categories!.includes(o.category));
        }

        // Championnat
        if (filters.championships && filters.championships.length > 0) {
            res = res.filter(
                (o) => !!o.championship && filters.championships!.includes(o.championship)
            );
        }

        // Classe d’âge
        if (filters.ageClasses && filters.ageClasses.length > 0) {
            res = res.filter((o) => !!o.ageRange && filters.ageClasses!.includes(o.ageRange));
        }
        ...
        if (filters.positions && filters.positions.length > 0) {
            res = res.filter((o) => !!o.position && filters.positions!.includes(o.position));
        }
        ...
        <SafeAreaView className="flex-1 bg-black">
            ...
            <View className="px-4 pb-2">
                ...
                <View className="flex-row items-center justify-between mb-4">
                    <View className="flex-row items-center">
                        ...
                        <Text className="text-white text-2xl font-bold ml-2">Rechercher une offre</Text>
                    </View>
                    <TouchableOpacity onPress={() => setFilterVisible(true)} className="p-2 rounded-xl">
                        <Ionicons name="filter-outline" size={26} color="#F97316" />
                    </TouchableOpacity>
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    onEndReachedThreshold={0.3}
                    contentContainerStyle={{ paddingBottom: 24, paddingHorizontal: 16 }}
                    ListEmptyComponent={<Text className="text-gray-500 text-center mt-10">Aucune offre trouvée</Text>}
                    renderItem={({ item }) => (
                        <Pressable
                            onPress={() =>
                        ...
                        >
                            ...
                            {!!item.location && <Text className="text-gray-300 mb-2">📍 {item.location}</Text>}
                            ...
                            <View className="flex-row flex-wrap gap-2">
                                ...
                                {!!item.championship && (
                                    <View className="bg-indigo-600/80 px-3 py-1 rounded-full">
                                        <Text className="text-white text-xs">{item.championship}</Text>
                                    </View>
                                )}
                                {!!item.ageRange && (
                                ...
                                {(() => {
                                    const effDept = item.department || clubsIndex[item.clubUid]?.department || "";
                                    return effDept ? (
                                        <View className="bg-gray-700 px-3 py-1 rounded-full">
